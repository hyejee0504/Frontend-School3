...
* HTML은 콘텐츠와 골격, CSS는 웹페이지의 레이아웃, JavaScript는 웹페이지의 동작
    ...
    * ECMA-인터내셔널 공식 사이트 : https://www.ecma-international.org/ - 업데이트 상황을 볼 수 있음 (굳이 볼 필요x)
...
* 보통 body의 맨 끝
앞에도 넣을 수 있지만 미리 로드해야하는 것들 (시간이 오래 걸리면 안됨)
...
## 코드 구조
1. 문(statement)은 세미콜론으로 구분(세미콜론을 붙이지 않는 곳도 있습니다. - 회사마다 컨센션이 다르다)
...
## 클래스(함수 설탕 : syntax sugar X - 클래스만의 문법이 많이 생김)
생성자함수와 비슷하지만 똑같이 사용 못함!
...
## 사용자와 상호작용
* prompt('hello') //prompt로 받은 것은 다 string이다.
* comfirm('hello')
* alert('hello')
